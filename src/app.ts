...
class App {
  ...
  constructor(app: express.Application, port: number, controllers: BaseController[], env = 'DEV') {
    dotenv.config();
  ...
  private initializeControllers(controllers: BaseController[]) {
    ...
    this.app.get('/country/all', routeCache(redisClient), async (req, res) => {
      redisClient.setex(req.originalUrl, 60 * 60 * 24 * 30 * 12, JSON.stringify(countries));
    ...
    this.app.get('/banks', routeCache(redisClient), async (req, res) => {
      ...
      let _data;
      try {
        const { data } = await axios.default.get(`${process.env.PAYSTACK_BEP}/bank`, config);
        _data = data
      } catch (error) {
        res.status(500).send({error: error.message, message: 'Could not get banks'});
        return;
      }
