...
export class MediaController implements BaseController {
    ...
    private initImageUploader(router: express.Router) {
        ...
        router.post('/upload/image', validateUser, upload.single('image'), async (req, res) => {
            try {
                const { public_id, width, height, secure_url:image } = await this.cloudinary.uploader.upload(req.file.path, {
                    allowed_formats: [
                        'png',
                        'jpg'
                    ],
                ...
                res.send({
                    image: {
                        thumb: this.cloudinary.url(public_id, { width: 150, height: 150, crop: "fill" }),
                        width, height, image
                    }
                })
            } catch (e) {
